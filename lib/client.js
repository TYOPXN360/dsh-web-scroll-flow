...
`;
		...
		const CONTENT_SELECTOR = `[class*="_markdown_"], [data-dsh-markdown], .markdown, [data-variant="think"][aria-expanded="true"] > * > [class*="thinkBody"], [class*="thinkBody"]`;
		/**
		...
		*/
		...
		//#endregion
		//#region src/client/think-markdown.ts
		/**
		...
		*/
		...
		function thinkMarkdownToHtml(text) {
		...
		//#endregion
		//#region src/client/think-body-markdown.ts
		/**
		...
		*/
		...
		var ThinkBodyMarkdown = class {
			...
			dispose() {
				...
				this.observer?.disconnect();
				this.observer = null;
				for (const el of this.root.querySelectorAll(`[${MARKED_ATTR}]`)) {
					const original = this.lastText.get(el);
					if (original !== void 0) el.textContent = original;
					el.removeAttribute(MARKED_ATTR);
				}
			}
			sync() {
				const bodies = this.root.querySelectorAll("[class*=\"thinkBody\"]");
				for (const body of bodies) {
					const text = body.textContent ?? "";
					if (text === this.lastText.get(body)) continue;
		...
		function createTypewriter(element, _mode, controller) {
			...
			const onContentChange = () => {
				controller.suppressEntryFor(1e3);
			};
		...
		*/
		function apply(ctx) {
			...
			thinkStyle.textContent = ["[data-variant=\"think\"][data-state=\"running\"] [class*=\"summary\"]{scroll-behavior:smooth}", "[data-variant=\"think\"][data-state=\"running\"] [class*=\"thinkBody\"]{scroll-behavior:smooth}"].join("");
