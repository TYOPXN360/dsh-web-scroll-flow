		...
		*/
		var TypewriterController = class {
			...
			attach() {
				...
				this.ensureStyleTag();
				for (const markdown of this.markdowns()) this.lastSeenByMarkdown.set(markdown, markdown.textContent ?? "");
			...
			*/
			renderOverlay(session) {
				...
				paragraphs.forEach((paragraph, index) => {
					...
					if (index < paragraphs.length - 1) p.style.margin = `0 0 ${session.paragraphGap}px`;
		...
		var ScrollFlowPolicy = class {
			...
			*/
			...
			adopt(host) {
				...
				if (mode === "off" || mode === "gentle" || mode === "medium") this.followMode.set(mode);
