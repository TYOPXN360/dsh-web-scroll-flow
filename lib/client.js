...
`;
		...
		*/
		var TypewriterController = class {
			...
			markdowns() {
				return Array.from(this.flow.querySelectorAll(MARKDOWN_SELECTOR)).filter((markdown) => !markdown.classList.contains(TYPEWRITER_OVERLAY_CLASS));
			}
			textNodes(root) {
				const nodes = [];
			...
			markdownIndexOf(el) {
				...
				return Array.from(container.querySelectorAll(MARKDOWN_SELECTOR)).filter((markdown) => !markdown.classList.contains(TYPEWRITER_OVERLAY_CLASS)).indexOf(el);
			}
		...
		*/
		...
		//#endregion
		//#region src/client/thinking-marquee.ts
		/**
...
`;
		var ThinkingMarquee = class {
			...
			dispose() {
				...
				this.styleEl?.remove();
				this.styleEl = null;
				for (const el of this.root.querySelectorAll("[data-marquee]")) {
			...
			sync() {
				const summaries = Array.from(this.root.querySelectorAll(SUMMARY_SELECTOR));
			...
			measure(el) {
				const expanded = el.closest("[aria-expanded]")?.getAttribute("aria-expanded") === "true";
