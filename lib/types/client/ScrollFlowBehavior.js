...
import { logDebug, tickDebugFrame } from "./debug-logger.js";
...
function isRowLevel(el) {
    if (el.hasAttribute('data-chat-flow-key') || el.hasAttribute('data-follow-end'))
        return true;
...
function findExpandedThinkBody(think) {
    if (think.querySelector('[data-follow-end]') !== null)
        return null;
    let best = null;
    for (const d of think.querySelectorAll('div')) {
        if (d === think)
            continue;
        if (d.hasAttribute('data-chat-flow-key'))
            continue;
        if (d.closest('[data-variant="think"]') !== think)
            continue;
        best = d;
    }
    return best;
}
const thinkExpandedOnce = new WeakSet();
...
function animateThinkExpandHeight(body) {
    ...
    const anim = body.animate([{ height: '0px' }, { height: `${target}px` }], { duration: 220, easing: 'ease-out' });
    anim.onfinish = () => {
        body.style.height = '';
        body.style.overflow = '';
    };
...
function animateThinkSlideIn(body) {
    ...
    body.animate([{ opacity: 0, transform: 'translateY(12px)' }, { opacity: 1, transform: 'translateY(0)' }], { duration: 200, easing: 'ease-out' });
}
...
function fadeIn(el, delayMs = 0) {
    ...
    try {
        el.animate([{ opacity: 0 }, { opacity: 1 }], { duration: FADE_IN_DURATION, easing: 'ease-out', delay: delayMs, fill: 'both' });
...
function revealThinkBodyLines(el) {
    if (el.querySelector('[data-scroll-flow-row]') !== null)
...
const fadedAt = new WeakMap();
function fadeOnce(el, delayMs = 0) {
    const last = fadedAt.get(el) ?? -Infinity;
    if (performance.now() - last < FADE_DEDUP_WINDOW_MS)
        return;
 ...
 */
export function advanceFollowScroll(el) {
 ...
 */
export function ScrollFlowBehavior({ useEnabled, useSession }) {
    ...
    useEffect(() => {
        ...
        const onScroll = (event) => {
            const port = scrollportRef.current;
            if (port === null || event.target !== port)
                return;
            if (!activeRef.current && !suppressRef.current)
                return;
            ...
            if (top < lastTopRef.current - 0.5)
                followingRef.current = false;
            if (gap <= AT_BOTTOM_GAP)
                followingRef.current = true;
    ...
    return _jsx("div", { ref: anchorRef, className: css.anchor, "aria-hidden": "true" });
}
//# sourceMappingURL=ScrollFlowBehavior.js.map