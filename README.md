...
3. **流式逐字打字机（原生 / 覆盖层两种模式）**：思维链与正文在流式输出时以字为单位逐字显示，带闪烁光标。
   - 原生：直接截断原始 Markdown 文本节点，无双份 DOM，段落 / 行距天然一致。
   - 覆盖层：在原内容上叠加模拟层逐字显示。
   段落间距与完成后的 Markdown 一致，打完字不跳变；大段文本按公式自动提速。初次加载 / 刷新页面时历史消息不会全部重新打字。
4. **设置面板可调**：General 设置中新增四项，选择持久化到浏览器：
...
- 回弹软增益参考距离（16px）、弹簧刚度 / 阻尼 / 滚轮灵敏度 / 松手延迟
